...
#### 유량제어
- **Redis Sorted Set 슬라이딩 윈도우**: 요청 시각(ms)을 score로 저장, 항상 "지금 기준 과거 60초" 동적 계산
- Lua 스크립트로 `ZREMRANGEBYSCORE` → `ZCARD` → `ZADD` 원자 처리
- 5회/분 제한, HTTP 429 + `X-RateLimit-Remaining`, `X-RateLimit-Reset` 헤더 응답
- SSE/상태 조회 엔드포인트 제외 처리
...
| # | 항목 | 우선순위 | 설명 |
|---|---|---|---|
| 1 | `deleteByCourseId` 최적화 | 낮음 | 현재 findAll + deleteAll 방식 → 벌크 쿼리로 개선 가능 |
